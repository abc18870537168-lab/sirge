    ...
    try:
        ...
        with connection:
            ...
            if cursor.fetchone()['cnt'] == 0:
                ...
                # 【修改完毕】已全部替换为“iPhone 18 系列”
                cursor.execute(
                    "INSERT INTO rumors (category, content, source) VALUES ('处理器', 'iPhone 18 系列将全系标配台积电 2nm 工艺的 A20 芯片，性能提升巨大', '郭明錤')")
                r1_id = cursor.lastrowid

                cursor.execute(
                    "INSERT INTO rumors (category, content, source) VALUES ('屏幕外观', 'iPhone 18 系列将彻底取消实体按键，采用全固态压感设计', '彭博社')")
                cursor.execute(
                ...
                cursor.execute(
                    "INSERT INTO predictions (user_id, rumor_id, vote_type, confidence_level) VALUES (?, ?, 1, 4)",
                    (u1, r1_id))
                cursor.execute(
                    "INSERT INTO predictions (user_id, rumor_id, vote_type, confidence_level) VALUES (?, ?, 1, 10)",
                    (u2, r1_id))
...
def handle_exception(e):
    ...
    <div style="background:#111; color:white; padding:40px; font-family:sans-serif;">
        ...
        <p style="font-size:18px;">别慌！请把下面黑框里的红字全选，直接发给 AI：</p>
