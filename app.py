    ...
    try:
        ...
        with connection:
            ...
            if cursor.fetchone()['cnt'] == 0:
                ...
                # 因为上面清零了序列，这里的 rumor_id=1 绝对安全了！
...
def purge_all():
    ...
    try:
        ...
        with connection:
            cursor = connection.cursor()
            # 级联删除发挥作用：清空 rumors 时，predictions 里相关的打分记录也会瞬间蒸发
...
def restore_data():
    ...
    try:
        ...
        with connection:
            cursor = connection.cursor()
            cursor.execute("DELETE FROM rumors") 
            # 【核心修复区】：清空 SQLite 内部的记账本，让下次插入的 ID 从 1 重新开始
            cursor.execute("DELETE FROM sqlite_sequence WHERE name='rumors'")
    except:
        pass
